SpeakTypesSettings = {
  none = {},
  say = { speakType = MessageModes.Say, color = '#ffffff', npChat = true },
  ...
  channelYellow = { speakType = MessageModes.Channel, color = '#FFFF00', npcChat = true  },
  channelWhite = { speakType = MessageModes.ChannelManagement, color = '#FFFFFF', npcChat = true  },
  channelRed = { speakType = MessageModes.GamemasterChannel, color = '#F55E5E', npcChat = true  },
  channelOrange = { speakType = MessageModes.ChannelHighlight, color = '#F6A731', npcChat = true  },
...
function getNewHighlightedText(text, defaultColor, highlightColor1, highlightColor2, highlightColor3)
  local result = {}
  local parts = text:split("&") -- Primeiro, dividimos por &

  for i, part in ipairs(parts) do
    if i % 2 == 1 then
      -- Parte fora de &, processamos #, %, $ e {}
      local subParts = part:split("##")
      for j, subPart in ipairs(subParts) do
        if j % 2 == 1 then
          -- Parte fora de #, processamos %, $ e {}
          local percentParts = subPart:split("%%")
          for l, percentPart in ipairs(percentParts) do
            if l % 2 == 1 then
              local dollarParts = percentPart:split("$")
              for m, dollarPart in ipairs(dollarParts) do
                if m % 2 == 1 then
                  -- Processar {}
                  local braceParts = dollarPart:split("{")
for n, bracePart in ipairs(braceParts) do
  if n == 1 then
    table.insert(result, bracePart)
    table.insert(result, defaultColor)
  else
    local innerParts = bracePart:split("}")
    if #innerParts > 1 then
      -- Verifica se há tag color=
      local colorTag, content = innerParts[1]:match("^color=(%S+)%s+(.*)")
      if colorTag and content then
        local colorHexMap = {
          red = "#FF0000",
          blue = "#0000FF",
          green = "#00FF00",
        ...
        local colorCode = colorHexMap[colorTag:lower()] or colorTag
        table.insert(result, content)
        table.insert(result, colorCode)
      else
        table.insert(result, innerParts[1]) -- Texto dentro de {}
        table.insert(result, highlightColor1) -- Cor para {}
      end
      table.insert(result, innerParts[2]) -- Texto após }
      table.insert(result, defaultColor)
    else
      table.insert(result, innerParts[1])
      table.insert(result, defaultColor)
    end
  end
end
                  ...
                  table.insert(result, dollarPart)
                  table.insert(result, "#ffffff") -- Cor para $
                end
              end
            else
              table.insert(result, percentPart)
              table.insert(result, "#0a0a00") -- Cor para %
            end
          end
        else
          -- Parte dentro de #, aplicamos a segunda cor
          table.insert(result, subPart)
          table.insert(result, "#ff0000")
        end
      end
    else
      -- Parte dentro de &, aplicamos a terceira cor
      table.insert(result, part)
      table.insert(result, highlightColor3)
    end
  end
...
-- text = "a"
  ...
  print("[DEBUG] Abortando - tab inválido ou texto vazio.")
  return
end
...
-- Só processa se NÃO for NpcChat ou Private
print("[DEBUG] Checando tipo de fala...")
print(" - npcChat:", speaktype.npcChat)
print(" - private:", speaktype.private)
print(" - npChat:", speaktype.npChat)

if not speaktype.npcChat and not speaktype.private and not speaktype.npChat then
  local rarityPrefixes = {
    ["[N]"] = "#FFFFFF", -- Branco (Normal)
    ["[R]"] = "#0033FF", -- Azul Escuro (Raro)
    ["[E]"] = "#9933FF", -- Roxo (Épico)
    ["[L]"] = "#FFD700", -- Dourado (Lendário)
    ["[M]"] = "#FF0000"  -- Vermelho (Mítico)
  }

  if text then
    local prefix = text:match("(%[[A-Z]%])")
    if prefix and rarityPrefixes[prefix] then
      local color = rarityPrefixes[prefix]
      if not text:find("{color=") then
        local coloredPrefix = string.format("{color=%s %s}", color, prefix)
        local safePrefix = prefix:gsub("%[", "%%["):gsub("%]", "%%]")
		
        text = text:gsub(safePrefix, coloredPrefix, 1)
		print("[DEBUG] Label final:", text)
...
if highlightData and #highlightData > 0 then
  ...
  label:setText(text) -- fallback
end

  label:setFont('sono_bold_border_14')
  if speaktype.npChat then
  label:setFont('sono_bold_border_14')
  end
...
function applyMessagePrefixies2(name, level, message)
  if not name or #name == 0 then
    return message
  end

  -- Verifica se é staff (ADM, GM, CM ou similar)
  local isStaff = name:upper():find("ADM") or name:upper():find("GM") or name:upper():find("CM")

  -- Aplica o prefixo STAFF se for membro da equipe
  local displayName = isStaff and "STAFF " .. name or name

  -- Formata a mensagem com ou sem nível
  if modules.client_options.getOption('showLevelsInConsole') and level > 0 then
    return "" .. displayName .. " [" .. level .. "]: " .. message
  else
...
function applyMessagePrefixies(name, level, message)
  ...
  if modules.client_options.getOption('showLevelsInConsole') and level > 0 then
    return "##[Private Message]## " .. displayName .. " [" .. level .. "]: " .. message
  else
    return "##[Private Message]## " .. displayName .. ": " .. message
  end
  end
end
...
if speaktype.npChat then
  local composedMessage = applyMessagePrefixies2(name, level, message)
  end
